          ...
          for(Function::iterator bb = F->begin(), e = F->end(); bb != e; ++bb) {                
            for (BasicBlock::iterator i = bb->begin(), e = bb->end(); i!=e; ++i) {                    
              ...
              if (CallInst* ci = dyn_cast<CallInst>(i))  {
                ...
                if (dyn_cast<Function>(F) != func_log_msg) { // Avoid adding call instruction inside func_log_msg
                    IRBuilder<> Builder(i);
                    Value *param1 = Builder.CreateGlobalStringPtr(callerToCallee.c_str(), ".str");
                    CallInst* callLogMsg = Builder.CreateCall(func_log_msg, param1);
                }
              }
            } 
          }      
